...
class Ui(QtWidgets.QMainWindow):
    ...
    class EnterSeedPhraseWindow(BaseWindow):
        ...
        def textEditTextChanged(self):
            try:
                Program().check_phrase(self.form.seedTextEdit.toPlainText())
    ...
    class CardConnectionWindow(BaseWindow):
        def __init__(self):
            ...
            try:
                for card_type in Program.card_type_list(Program()):
                    self.form.cardTypesList.addItem(card_type)
                for card_reader in Program.cardreaders_list(Program()):
        ...
        def updateButtonPressed(self):
            null_item = self.form.readersList.itemText(0)
            self.form.readersList.clear()
            self.form.readersList.addItem(null_item)
            try:
                for card_reader in Program.cardreaders_list(Program()):
                    self.form.readersList.addItem(card_reader)
            except Program.Warning as warning:
        ...
        def readersListIndexChanged(self, index):
            if index > 0:
                Program().set_current_reader(self.form.readersList.itemText(index))
                self.form.cardTypesList.setEnabled(True)
            elif index == 0:
                Program().set_current_reader(None)
                self.form.cardTypesList.setEnabled(False)
                self.form.cardTypesList.setCurrentIndex(0)

        def cardTypesListIndexChanged(self, index):
            if index > 0:
                try:
                    Program().set_card_type(self.form.cardTypesList.itemText(index))
                except:
                    self.form.cardTypesList.setCurrentIndex(0)
                    QtWidgets.QMessageBox.critical(self,
                                                   'Error!',
                ...
                self.form.readersList.setEnabled(False)
                self.form.nextButton.setEnabled(True)
            elif index == 0:
                Program().set_card_type(None)
                self.form.updateButton.setEnabled(True)
                self.form.readersList.setEnabled(True)
                self.form.nextButton.setEnabled(False)

    class EncryptSeedPhraseWindow(BaseWindow):
        def __init__(self):
            super().__init__(EncryptSeedPhrase_form)
            for enc_alg in Program().algorithms_list():
        ...
        def algorithmsListIndexChanged(self, index):
            if index > 0:
                if self.form.labelCorrect.text() != 'Very large contact information!':
        ...
        def textEditTextChanged(self):
            ...
            if len(self.form.contactDataTextEdit.toPlainText().encode('utf8')) > 20:
                ...
                if self.form.algorithmsList.currentIndex() > 0:
                    self.form.encryptButton.setEnabled(True)
    ...
    def encrypt_seed_phrase(self):
        ...
        self.seed_phrase = None
        write_func = lambda: Program().write_seed_phrase_to_card(seed_phrase,
    ...
    def go_next_action_card_connection(self):
        if self.program_mode == Ui.ProgramMode.Write:
            ...
            read_metadata_func = lambda: Program.read_card_data(Program())
            ...
            your_seed_phrase_func = lambda: QtWidgets.QMessageBox.information(self,
                                                                              'Successful!',
                                                                              f'Your seed phrase: {Program.read_card_data(Program(), password)}',
                                                                              QtWidgets.QMessageBox.Close)
            try:
                read_metadata_func()
                ...
                if ok:
                    try:
                        ...
                        password, ok = verify_encrypt_password_func()
                        if ok and password:
                            Program.read_card_data(Program(), password)
                            your_seed_phrase_func()
