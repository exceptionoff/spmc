...
import bip39
from crypt_engine.crypto_algorithms import (
    encryptAlgs,
...
class CardMarkup:
    ...
    def info_pack_bytes(cls, info: FieldStructure) -> bytes:
        data = (
            listTypeCards[info.card_type].program_id
    ...
    def info_bytes_unpack(cls, bytes_: bytes) -> FieldStructure:
        ...
        try:
            ...
            size_enc_seed = cls._size_encrypted_seed(
                encryptAlgs[enc_alg_name], bip39.get_entropy_bits(count_words_seed) // 8
            )
            size_iv = encryptAlgs[enc_alg_name].iv_size

            size_without_checksum = (
                cls.FieldStructure.header_size + size_enc_seed + size_iv
            )
            if (
                sha256(bytes_[:size_without_checksum]).digest()[:4]
                != bytes_[size_without_checksum : size_without_checksum + 4]
            ):
    ...
    def _card_name_to_bytes(cls, card_name: str):
        ...
        return b"\x80".join(
            [card_name.encode("utf8"), b"\x00" * (20 - len(card_name_b))]
        )
    ...
    def _size_encrypted_seed(cls, enc_alg: EncryptAlgHeader, seed_size: int):
        if ("ECB" in enc_alg.name or "CBC" in enc_alg.name) and (
            seed_size % enc_alg.block_len != 0
        ):
