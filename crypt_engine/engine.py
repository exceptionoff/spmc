...
class Cipher:
    ...
    def _init_encrypter_decrypter(self):
        ...
        if 'GOST_34_13_64' in self.enc_alg.name or 'GOST_34_13_128' in self.enc_alg.name:
            ...
            if 'ECB' in self.enc_alg.name:
                ...
                self.encrypter = lambda data: ecb_encrypt(EncAlgorithm(self._key).encrypt,
                                                          self.enc_alg.block_len,
                                                          data)
                self.decrypter = lambda data: ecb_decrypt(EncAlgorithm(self._key).decrypt,
                                                          self.enc_alg.block_len,
                                                          data)
            if 'CTR' in self.enc_alg.name:
                from pygost.gost3413 import ctr

                self.encrypter = lambda data: ctr(EncAlgorithm(self._key).encrypt,
                                                  self.enc_alg.block_len,
                                                  data,
                                                  self._iv)
            ...
            if 'OFB' in self.enc_alg.name:
                ...
                self.encrypter = lambda data: ofb(EncAlgorithm(self._key).encrypt,
                                                  self.enc_alg.block_len,
                                                  data,
                                                  self._iv)
                self.decrypter = self.encrypter
            ...
            if 'CBC' in self.enc_alg.name:
                ...
                self.encrypter = lambda data: cbc_encrypt(EncAlgorithm(self._key).encrypt,
                                                          self.enc_alg.block_len,
                                                          data,
                                                          self._iv)
                self.decrypter = lambda data: cbc_decrypt(EncAlgorithm(self._key).decrypt,
                                                          self.enc_alg.block_len,
                                                          data,
                                                          self._iv)
            ...
            if 'CFB' in self.enc_alg.name:
                ...
                self.encrypter = lambda data: cfb_encrypt(EncAlgorithm(self._key).encrypt,
                                                          self.enc_alg.block_len,
                                                          data,
                                                          self._iv)
                self.decrypter = lambda data: cfb_decrypt(EncAlgorithm(self._key).encrypt,
                                                          self.enc_alg.block_len,
                                                          data,
                                                          self._iv)
        ...
        if '3DES' in self.enc_alg.name or 'AES-256' in self.enc_alg.name:
            ...
            if 'ECB' in self.enc_alg.name:
                self.encrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_ECB).encrypt(data)
                self.decrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_ECB).decrypt(data)
            if 'CTR' in self.enc_alg.name:
                self.encrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_CTR,
                                                               nonce=self._iv).encrypt(data)
                self.decrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_CTR,
                                                               nonce=self._iv).decrypt(data)
            if 'OFB' in self.enc_alg.name:
                self.encrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_OFB,
                                                               iv=self._iv).encrypt(data)
                self.decrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_OFB,
                                                               iv=self._iv).decrypt(data)
            if 'CBC' in self.enc_alg.name:
                self.encrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_CBC,
                                                               iv=self._iv).encrypt(data)
                self.decrypter = lambda data: EncAlgorithm.new(self._key,
                                                               EncAlgorithm.MODE_CBC,
                                                               iv=self._iv).decrypt(data)
    ...
    class WrongKeyLength(Exception):
        def __init__(self, actual_value: int, excepted_value: int):
            super().__init__()
            self.msg = f"The key length must be {excepted_value} bytes (actual {actual_value})"
